...
<nav
  ...
  class={classNames(
    ...
    shadow ? 'bg-white/95 backdrop-blur-sm shadow-sm dark:bg-gray-900/95 border-b border-gray-200 dark:border-gray-800' : 'bg-white/95 backdrop-blur-sm dark:bg-gray-900/95'
  )}
>
  <!-- China-specific brand logo -->
  <div>
    <div class="inline-flex items-center gap-2 rounded-md border border-gray-200 p-2 dark:border-secondary">
      <a class="shrink-0" href="/">
        <img class="h-10" src="/logo.svg" alt="HackHarvard" />
      </a>
    </div>
  </div>
      ...
      {#each pages as page}
        <a
          class={classNames(
            ...
            !page.isExternal && hash === page.href
              ? shadow 
          ...
          href={page.href}
          on:click={event => scrollToSection(event, page.href, page.isExternal)}
        >
          {page.name}
        </a>
      {/each}
  ...
  >
    {#each pages as page}
      <a
        ...
        href={page.href}
        on:click={event => {
          scrollToSection(event, page.href, page.isExternal)
          open = false
        }}
      >
        {page.name}
